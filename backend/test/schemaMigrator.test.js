...
test('legacy payment method enums are widened for cheque and other receipts', async () => {
  const tables = {
    sales_payments: { paying_method: { type: "ENUM('cash','card','bank_transfer','credit')" } },
    customer_payments: { paying_method: { type: "ENUM('cash','card')" } },
  };
  const changed = [];
  const queryInterface = {
    async describeTable(tableName) { return tables[tableName] ? { ...tables[tableName] } : null; },
    async changeColumn(tableName, columnName, definition) {
      tables[tableName][columnName] = definition;
      changed.push(`${tableName}.${columnName}`);
    },
  };
  const fakeSequelize = {
    modelManager: { models: [] }, models: {}, getQueryInterface: () => queryInterface,
  };

  const { normalizePaymentMethodColumns } = loadMigratorWith(fakeSequelize);
  await normalizePaymentMethodColumns({ verbose: false });
  assert.deepEqual(changed, ['sales_payments.paying_method', 'customer_payments.paying_method']);
});
